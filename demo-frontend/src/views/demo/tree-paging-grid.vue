...
export default {
  ...
  methods: {
    ...
    async featchTreeData () {
      ...
      // flatData.forEach(father => {
      //   let childs = flatData.filter(child => father.id === child.parentId) // 请确保接口输出数据用parentId、id来表示父子级关系
      //   childs.length > 0 ? (father.children = childs) : (father.children = [])
      // })
      // this.tree.data[0].children = flatData.filter(father => !flatData.find(item => item.children.find(child => child === father)))
    },
    // 树节点点击
    onTreeNodeClick (data) {
      this.pagging.pageNum = 1
      this.tree.selectedData = data
      this.featchData()
    },
    // 获取列表数据
    async featchData () {
      ...
      //     ...this.search,
      //     treeNodeId:this.tree.selectedData.id
      //   }
      // })
    ...
    async validateBeforeAdd () {
      // if (!this.curTarget) {
      //   this.$message.info('新增校验不通过，不允许新增！')
    ...
    async validateBeforeDelete (row) {
      ...
      //   this.$message.info('删除校验不通过，不允许删除！')
      //   return Promise.reject(new Error())
      // }
      return Promise.resolve()
    },
    ...
    resize () {
      this.grid.height = this.$el.clientHeight - this.$el.querySelector('.search-bar').clientHeight - this.$el.querySelector('.tool-bar').clientHeight - this.$el.querySelector('.pagination').clientHeight
    }
  }
}
</script>
